...
const VARIABLES = [
  '{{CustomerName}}', '{{CompanyName}}', '{{InvoiceNumber}}', 
  '{{SubscriptionName}}', '{{PlanName}}', '{{Amount}}', 
  '{{DueDate}}', '{{PaymentDate}}'
];

export default function EmailTemplateEditor({ notification, categoryName, onBack }) {
  const [formData, setFormData] = useState({
    ...
    body: notification?.body || 'Hi {{CustomerName}},\n\nWelcome to ManageMyOPZ!\n\nThanks,\nManageMyOPZ Team'
  });
    ...
    <div className="p-8 max-w-5xl mx-auto font-sans pb-24">
      ...
      <div className="flex items-center gap-4 mb-8 pb-4 border-b border-stone-200">
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold text-stone-900">{formData.name || 'New Template'}</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-lg shadow-sm border border-stone-200">
            <h2 className="text-lg font-semibold text-stone-900 mb-4">Template Settings</h2>
            
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">Notification Name</label>
                <input 
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">Email Subject</label>
                <input 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Sender Name</label>
                  <input 
                ...
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Sender Email</label>
                  <input 
                    type="email" name="senderEmail" value={formData.senderEmail} onChange={handleChange}
                    className="w-full px-3 py-2 border border-stone-300 rounded-md outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  />
                </div>
              </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-stone-200 flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold text-stone-900">Email Body</h2>
            </div>
            ...
            <div className="border border-stone-300 rounded-md overflow-hidden flex flex-col min-h-[300px]">
              ...
              <div className="bg-stone-50 border-b border-stone-300 p-2 flex gap-2">
                <button className="px-2 py-1 text-sm font-bold text-stone-700 hover:bg-stone-200 rounded">B</button>
                <button className="px-2 py-1 text-sm italic text-stone-700 hover:bg-stone-200 rounded">I</button>
                <button className="px-2 py-1 text-sm underline text-stone-700 hover:bg-stone-200 rounded">U</button>
        ...
        {/* Sidebar: Variables & Actions */}
        <div className="space-y-6">
          
          <div className="bg-white p-6 rounded-lg shadow-sm border border-stone-200">
            <h3 className="text-sm font-bold text-stone-900 mb-3 uppercase tracking-wide">Dynamic Variables</h3>
            <p className="text-xs text-stone-500 mb-4">Click to insert into the email body.</p>
