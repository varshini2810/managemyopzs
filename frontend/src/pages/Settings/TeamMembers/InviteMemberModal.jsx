...
export default function InviteMemberModal({ isOpen, onClose, onSuccess }) {
  ...
  const onSubmit = async (data) => {
    try {
      const res = await api.post('/settings/team-members', data); // changed from invite to root per prompt
      
      if (data.role === 'ADMIN' && selectedPerms.length > 0) {
        await api.put(`/settings/team-members/${res.data.data.id}/permissions`, { permissions: selectedPerms });
      }
