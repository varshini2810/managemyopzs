...
export default function CreateWebhookModal({
  ...
  const onSubmit = async (data) => {
    try {
      /*  Process comma-separated events into JSON array string  */ const eventsList =
        data.events
          ? data.events
              .split(",")
              .map((e) => e.trim())
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditing ? "Edit Webhook" : "Add Webhook Endpoint"}
    >
      {" "}
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            {" "}
            Comma-separated list of event names. Leave blank to receive all
            events.{" "}
          </p>{" "}
        </div>{" "}
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            {" "}
            If provided, we will include a signature in the X-Webhook-Signature
            header.{" "}
          </p>{" "}
        </div>{" "}
