  ...
  paypal: {
    ...
    logo: (
      <svg viewBox="0 0 64 20" height="20" fill="none">
        <text x="0" y="15" fontFamily="Arial" fontWeight="bold" fontSize="14" fill="#003087">Pay</text>
        <text x="28" y="15" fontFamily="Arial" fontWeight="bold" fontSize="14" fill="#009cde">Pal</text>
      </svg>
    ),
  },
  gocardless: {
    ...
    logo: (
      <svg viewBox="0 0 90 20" height="20" fill="none">
        ...
        <text x="26" y="15" fontFamily="Arial" fontWeight="600" fontSize="12" fill="#1A1A1A">GoCardless</text>
      </svg>
    ),
  },
};
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div
          ...
          style={{ width: 56, height: 40, background: '#FAFAF9', border: '1px solid #E7E5E2' }}
        >
          {meta.logo}
        </div>
        <div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        {isConnected ? (
          <>
            ...
            <button className="btn-ghost btn-sm text-accent" style={{ color: '#2D5BFF' }}>
              Configure
            </button>
          </>
        ) : (
...
export default function PaymentGateways() {
  ...
  const fetchGateways = () => {
    setLoading(true);
    api.get('/settings/payment-gateways')
  ...
  const handleToggle = async (gateway) => {
    try {
      ...
      await api.put(`/settings/payment-gateways/${gateway.id}`, { status: newStatus });
      toast.success(`${gateway.name} ${newStatus === 'ACTIVE' ? 'enabled' : 'disabled'}`);
      fetchGateways();
    } catch {
  ...
  const handleConnect = async (gwId, name) => {
    try {
      await api.post('/settings/payment-gateways', { name, status: 'INACTIVE', config: {} });
    ...
    <div>
      ...
      <div className="px-8 py-6 max-w-3xl">
        <div className="mb-6">
          ...
          <p className="page-subtitle">Connect payment processors to accept customer payments</p>
        </div>

        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="rounded-lg h-20 skeleton" style={{ background: '#E7E5E2' }} />
            ))}
          </div>
        ) : (
          <div className="space-y-3">
            {Object.entries(GATEWAY_META).map(([id, meta]) => {
              const gw = gateways.find(g => (g.id || '').toLowerCase() === `gw_${id}` || (g.name || '').toLowerCase() === id);
              return (
                <GatewayCard
                  key={id}
                  meta={meta}
        ...
        >
          <strong className="text-ink">PCI DSS compliance:</strong>{' '}
          Opz never stores raw card data. All payment credentials are managed by your chosen gateway.
          API keys are encrypted at rest with AES-256 and only displayed once at setup.
        </div>
      </div>
    </div>
  );
