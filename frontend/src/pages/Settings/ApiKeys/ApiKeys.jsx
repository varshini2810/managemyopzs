...
import { Plus, Key, Copy, Trash2, Clock, ChevronRight, Check } from 'lucide-react';
    ...
    },
    {
      ...
      cell: (row) => row.lastUsedAt ? (
        <span className="flex items-center gap-1.5 text-sm text-muted tabular-nums">
          ...
          {new Date(row.lastUsedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
        </span>
      ) : (
  ...
  return (
    <div>
      <div className="module-header">
        ...
        <button className="btn-primary btn-sm" onClick={() => setIsModalOpen(true)}>
      ...
      <div className="px-8 py-6 max-w-5xl space-y-6">
        <div>
          ...
          <p className="page-subtitle">Manage keys used to authenticate API requests</p>
        </div>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-success-muted mb-4 max-w-lg" style={{ color: '#1A7F4E' }}>
        ...
        {!loading && keys.length === 0 ? (
          <div className="bg-surface rounded-lg flex flex-col items-center justify-center py-20" style={{ border: '1px solid #E7E5E2' }}>
            ...
            <h3 className="text-base font-semibold text-ink mb-1">No API Keys</h3>
            <p className="text-sm text-muted mb-6 text-center max-w-xs">
              Create a secret key to authenticate your backend servers or custom scripts with our API.
            </p>
