...
export default function AddRegionModal({ isOpen, onClose, onAdd, existingCodes = [] }) {
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-border">
          {filtered.length === 0 ? (
            <div className="px-4 py-8 text-center text-sm text-muted">No results for "{search}"</div>
          ) : (
              ...
              return (
                <button
                  ...
                  className={`w-full text-left flex items-center gap-3 px-4 py-2.5 transition-colors
                    ${alreadyAdded ? 'opacity-40 cursor-not-allowed' : 'hover:bg-stone-50 cursor-pointer'}
                    ${isSelected ? 'bg-blue-50' : ''}
                  `}
                >
                  <span className="text-lg leading-none">{country.flag}</span>
                  <span className={`text-sm flex-1 ${isSelected ? 'font-medium text-accent' : 'text-ink'}`}>
                    {country.name}
                  </span>
                  {alreadyAdded && (
                    <span className="text-2xs text-muted bg-stone-100 px-1.5 py-0.5 rounded">Added</span>
                  )}
