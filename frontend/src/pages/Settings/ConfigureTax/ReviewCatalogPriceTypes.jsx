...
export default function ReviewCatalogPriceTypes({ onBack }) {
  ...
  const handleUpdate = (currency, type) => {
    ...
    toast.success(
      `${currency} price type updated to Tax ${type.charAt(0).toUpperCase() + type.slice(1)}`,
    );
  };
  return (
    <div>
      ...
      <div className="px-8 py-8 max-w-3xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="page-subtitle mt-1">
            {" "}
            Select whether your prices for each currency are tax inclusive or
            tax exclusive.{" "}
          </p>{" "}
        </div>{" "}
        ...
        <p className="text-xs text-muted mt-4">
          {" "}
          Click a currency to change its tax price type. Changes take effect
          immediately for new invoices.{" "}
        </p>{" "}
      </div>{" "}
      {/* Price Type Modal */}{" "}
      <PriceTypeModal
        ...
        currentType={
          modal.currency
            ? priceTypes[modal.currency] || "exclusive"
            : "exclusive"
        }
