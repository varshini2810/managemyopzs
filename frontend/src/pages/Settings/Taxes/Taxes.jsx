...
import { Plus, Receipt, Settings as SettingsIcon, Trash2, ChevronRight } from 'lucide-react';
...
export default function Taxes() {
  ...
  const columns = [
    {
      header: 'Country',
      cell: (row) => <span className="font-medium text-ink uppercase">{row.country}</span>
    },
    {
      header: 'State/Province',
      cell: (row) => <span className="text-sm text-muted">{row.state || 'All States'}</span>
    },
    { header: 'Tax Name', accessor: 'taxName' },
    {
      header: 'Tax Rate',
      align: 'right',
      cell: (row) => <span className="tabular-nums font-medium text-ink">{row.taxRate}%</span>
    },
    {
      header: '',
      align: 'right',
      ...
      <div className="px-8 py-8 flex max-w-4xl mx-auto">
        <div className="flex-1 min-w-0 space-y-8">
          <div>
            ...
            <p className="page-subtitle">Configure global tax rules and regional tax rates</p>
          </div>

          {loading ? (
            <div className="space-y-4">
              <div className="skeleton h-32 rounded-lg" style={{ background: '#E7E5E2' }} />
              <div className="skeleton h-64 rounded-lg" style={{ background: '#E7E5E2' }} />
            </div>
          ) : (
            <>
              {/* Global Config */}
              <div className="card">
                ...
                <form onChange={handleSubmit(saveConfig)} className="space-y-0">
                  ...
                  <div className="flex justify-between items-center py-4 border-b border-border">
                    <div>
                      <div className="text-sm font-medium text-ink">Price Type</div>
                      <div className="text-xs text-muted mt-0.5">Determine if your catalog prices include or exclude tax</div>
                    </div>
                    <select className="select w-64" {...register('priceType')}>
                      <option value="exclusive">Exclusive (Add tax on checkout)</option>
                  ...
                  <div className="flex justify-between items-center py-4">
                    <div>
                      <div className="text-sm font-medium text-ink">Collect Tax ID</div>
                      <div className="text-xs text-muted mt-0.5">Ask business customers for VAT/Tax ID during checkout</div>
                    </div>
                    <Toggle 
              ...
              <div>
                ...
                {regions.length === 0 ? (
                  <div className="bg-surface rounded-lg flex flex-col items-center justify-center py-16" style={{ border: '1px solid #E7E5E2' }}>
                    ...
                    <h3 className="text-base font-semibold text-ink mb-1">No Tax Regions Defined</h3>
                    <p className="text-sm text-muted mb-6 text-center max-w-xs">
                      Add tax rates for specific countries or states where you are required to collect tax.
                    </p>
