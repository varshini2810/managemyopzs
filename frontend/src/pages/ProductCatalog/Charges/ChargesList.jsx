...
export default function ChargesList() {
  ...
  const fetchCharges = () => {
    setLoading(true);
    api
      .get(`/charges?page=${page}&size=20&search=${search}`)
      .then((res) => {
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div>
          ...
          <p className="page-subtitle">
            Configure ad-hoc and one-time charges
          </p>{" "}
        </div>{" "}
        {!loading && charges.length === 0 && search === "" ? (
          ...
          >
            ...
            <p className="text-sm text-muted mb-6 text-center max-w-xs">
              {" "}
              Charges allow you to bill customers for one-off services like
              setup fees, consulting, or hardware.{" "}
            </p>{" "}
            <button
