      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <div key={step.id} className="step-item">
            ...
            <div className={`step-dot ${done || past ? 'complete' : active ? 'active' : 'pending'}`}>
              {done || past ? <Check size={12} strokeWidth={2.5} /> : i + 1}
            </div>
            <div className="pt-0.5">
              <div className={`text-sm font-medium ${active ? 'text-ink' : past || done ? 'text-muted' : 'text-stone-400'}`}>
                {step.label}
              </div>
...
export default function CreatePlanModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSelfServe(false);
      setCheckout(false);
      api.get('/product-families/all')
        .then(res => setFamilies(res.data.data || []))
        .catch(() => {});
      api.get('/custom-fields?entityType=plan')
  ...
  const onSubmit = async (data) => {
    try {
      setSubmitting(true);
      const formattedCustomFields = Object.keys(customFieldValues).map(key => ({
        fieldId: key,
        fieldValue: customFieldValues[key]
      }));
      const payload = { ...data, displaySelfServe: selfServe, displayCheckout: checkout, customFieldValues: formattedCustomFields };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ border: '1px solid #E7E5E2', height: 'calc(100vh - 80px)', maxHeight: 680 }}
      >
        {/* Left — step rail */}
        <div
          className="shrink-0 flex flex-col"
          style={{ width: 220, background: '#FAFAF9', borderRight: '1px solid #E7E5E2', padding: '32px 24px' }}
        >
          <div className="mb-8">
            <div className="text-base font-semibold font-display text-ink tracking-tight">Create Plan</div>
            <div className="text-xs text-muted mt-1">Set up a new subscription plan</div>
          </div>
          <StepRail current={step} completed={completed} />
        </div>

        {/* Right — form content */}
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 min-w-0">
          {/* Header */}
          <div
          ...
          >
            <div>
              ...
              <p className="text-xs text-muted mt-0.5">{STEPS.find(s => s.id === step)?.desc}</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-8 py-7">
            ...
            {step === 'basic' && (
              <div className="space-y-5 max-w-lg">
                <div>
                  <label className="label">Product Family <span style={{ color: '#C0292B' }}>*</span></label>
                  <select className={`select ${errors.productFamilyId ? 'input-error' : ''}`}
                    {...register('productFamilyId', { required: 'Select a product family' })}>
                    <option value="">Select family…</option>
                    {families.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
                  </select>
                  {errors.productFamilyId && <p className="error-msg">{errors.productFamilyId.message}</p>}
                </div>
                <div>
                  <label className="label">Internal Name <span style={{ color: '#C0292B' }}>*</span></label>
                  <input type="text" className={`input ${errors.name ? 'input-error' : ''}`}
                    placeholder="e.g. Pro Monthly"
                    {...register('name', { required: 'Name is required' })} />
                  {errors.name && <p className="error-msg">{errors.name.message}</p>}
                  <p className="text-xs text-muted mt-1.5">Visible to admins only, not to customers</p>
                </div>
                <div>
            ...
            {step === 'customer' && (
              <div className="space-y-4 max-w-lg">
                <p className="text-sm text-muted">Control where customers can discover and select this plan.</p>
                {[
                  { key: 'selfServe', val: selfServe, set: setSelfServe, label: 'Display in Self-Serve Portal', desc: 'Allow existing customers to switch to this plan independently' },
                  { key: 'checkout', val: checkout, set: setCheckout, label: 'Display in Checkout', desc: 'Allow new customers to purchase via hosted checkout page' },
                ].map(opt => (
                  <div
                    key={opt.key}
              ...
              <div className="space-y-5 max-w-lg">
                <div>
                  ...
                  <div className="flex gap-3">
                    <input type="number" min="1" className="input" style={{ width: 90 }}
                      {...register('billingPeriod', { min: 1 })} />
                    <select className="select flex-1" {...register('billingPeriodUnit')}>
                ...
                <div>
                  ...
                  <div className="flex gap-3 items-center">
                    ...
                    <select className="select flex-1" {...register('trialPeriodUnit')}>
                      {['DAYS','WEEKS','MONTHS'].map(u => (
                        <option key={u} value={u}>{u.charAt(0) + u.slice(1).toLowerCase()}</option>
                      ))}
                    </select>
                  </div>
                  <p className="text-xs text-muted mt-1.5">Set to 0 for no trial period</p>
                </div>
              </div>
            )}
            ...
            {step === 'custom' && (
              <div className="space-y-5 max-w-lg">
                {customFields.length === 0 ? (
                  <p className="text-sm text-muted">No custom fields defined for Plans.</p>
                ) : (
                  customFields.map(cf => (
                    <div key={cf.id}>
                      <label className="label">{cf.fieldName} {cf.isRequired && <span style={{ color: '#C0292B' }}>*</span>}</label>
                      <input 
                        type="text" 
                        className="input" 
            ...
            {step === 'review' && (
              <div className="max-w-lg space-y-5">
                ...
                >
                  ...
                  {[
                    ['Name', formData.name || '—'],
                    ['Family', families.find(f => f.id === formData.productFamilyId)?.name || '—'],
                    ['Description', formData.description || '—'],
                    ['Billing', `${formData.billingPeriod} ${billingUnit.toLowerCase()}`],
                    ['Trial', trialPeriod > 0 ? `${trialPeriod} ${(formData.trialPeriodUnit || 'DAYS').toLowerCase()}` : 'No trial'],
