...
import CreatePlanModal from "./CreatePlanModal"; // Module sub-header
...
export default function PlansList() {
  ...
  const fetchPlans = () => {
    setLoading(true);
    api
      .get(`/plans?page=${page}&size=20&search=${encodeURIComponent(search)}`)
      .then((res) => {
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-sm text-muted tabular-nums">
          {" "}
          {row.trialPeriod > 0
            ? `${row.trialPeriod} ${row.trialPeriodUnit?.toLowerCase()}`
            : "—"}{" "}
        </span>
      ),
    },
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div>
          ...
          <p className="page-subtitle">
            Subscription plans with billing frequency and trial periods
          </p>{" "}
        </div>{" "}
        {!loading && plans.length === 0 && search === "" ? (
          ...
          >
            ...
            <p className="text-sm text-muted mb-6 text-center max-w-xs">
              {" "}
              Plans define your subscription offerings. Create your first plan
              to get started.{" "}
            </p>{" "}
            <button
