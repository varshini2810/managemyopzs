...
export default function AddonsList() {
  ...
  const fetchAddons = () => {
    setLoading(true);
    api
      .get(`/addons?page=${page}&size=20&search=${encodeURIComponent(search)}`)
      .then((res) => {
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-sm text-muted capitalize">
          {(row.pricingScheme || "").replace("_", " ").toLowerCase()}
        </span>
      ),
    },
    {
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div>
          ...
          <p className="page-subtitle">
            Optional products or services customers can add to their
            subscriptions
          </p>{" "}
        </div>{" "}
        {!loading && addons.length === 0 && search === "" ? (
          ...
          >
            ...
            <p className="text-sm text-muted mb-6 text-center max-w-xs">
              {" "}
              Add-ons let customers augment their subscriptions with optional
              extras.{" "}
            </p>{" "}
            <button
