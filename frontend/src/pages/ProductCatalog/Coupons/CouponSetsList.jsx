...
export default function CouponSetsList() {
  ...
  const fetchSets = () => {
    setLoading(true);
    api.get(`/coupons/sets?page=${page}&size=20&search=${search}`)
      .then(res => {
  ...
  useEffect(() => { fetchSets(); }, [page, search]);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Delete this coupon set?')) {
      try {
        ...
        fetchSets();
      } catch { toast.error('Failed to delete coupon set'); }
    }
  };
  ...
  const columns = [
    ...
    {
      header: 'Coupon ID',
      cell: (row) => <MonoId value={row.couponId} />,
    },
    {
      header: 'Codes Generated',
      align: 'right',
      cell: (row) => (
        <span className="text-sm font-medium text-ink tabular-nums">{row.totalCodes?.toLocaleString() || '0'}</span>
      ),
    },
    {
      header: '',
    ...
    <div>
      ...
      <div className="px-8 py-6 space-y-6">
        <div>
          ...
          <p className="page-subtitle">Generate bulk unique codes for a specific coupon</p>
        </div>
        ...
        {!loading && sets.length === 0 && search === '' ? (
          <div className="bg-surface rounded-lg flex flex-col items-center justify-center py-20" style={{ border: '1px solid #E7E5E2' }}>
            ...
            <h3 className="text-base font-semibold text-ink mb-1">No Coupon Sets</h3>
            <p className="text-sm text-muted mb-6 text-center max-w-xs">
              Coupon sets allow you to generate thousands of unique, single-use codes linked to a single master coupon.
            </p>
          </div>
        ) : (
          <DataTable
