...
export default function ProductFamiliesList() {
  ...
  const fetchFamilies = () => {
    setLoading(true);
    api.get(`/product-families?page=${page}&size=20&search=${search}`)
      .then(res => {
  ...
  useEffect(() => { fetchFamilies(); }, [page, search]);
  ...
  const columns = [
    ...
    },
    {
      header: 'Description',
      cell: (row) => (
        <span className="text-sm text-muted truncate max-w-xs block">{row.description || '—'}</span>
      ),
    },
    {
      header: 'Created',
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex items-center justify-end gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); setEditingFamily(row); setIsModalOpen(true); }}
  ...
  return (
    <div>
      <div className="module-header">
        ...
        <button className="btn-primary btn-sm" onClick={() => { setEditingFamily(null); setIsModalOpen(true); }}>
      ...
      <div className="px-8 py-6 space-y-6">
        <div>
          ...
          <p className="page-subtitle">Manage your top-level product categories</p>
        </div>
        ...
        {!loading && families.length === 0 && search === '' ? (
          <div className="bg-surface rounded-lg flex flex-col items-center justify-center py-20" style={{ border: '1px solid #E7E5E2' }}>
            ...
            <h3 className="text-base font-semibold text-ink mb-1">No Product Families Found</h3>
            <p className="text-sm text-muted mb-6 text-center max-w-xs">
              Product families group related plans and add-ons together.
            </p>
            <button className="btn-primary" onClick={() => { setEditingFamily(null); setIsModalOpen(true); }}>
      ...
      <CreateFamilyModal
        ...
        onSuccess={() => { fetchFamilies(); setIsModalOpen(false); }}
