...
export default function DocumentViewerModal({ isOpen, onClose, document }) {
  ...
  const isImage = document.type?.startsWith('image/') || document.url?.match(/\.(jpeg|jpg|gif|png)$/i);
  const isPdf = document.type === 'application/pdf' || document.url?.match(/\.pdf$/i);

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`View Document: ${document.name}`}>
      <div className="flex flex-col h-[70vh]">
        <div className="flex-1 bg-gray-100 rounded border border-gray-200 overflow-hidden flex items-center justify-center relative">
          {isImage ? (
            ...
            <div className="text-muted flex flex-col items-center">
              ...
              <a href={document.url} target="_blank" rel="noreferrer" className="btn-secondary mt-4">
        ...
        <div className="mt-4 flex justify-between items-center">
          ...
          <a href={document.url} download className="btn-primary">Download</a>
        </div>
      </div>
    </Modal>
  );
