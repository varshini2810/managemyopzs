...
export default function InvoiceAnalytics() {
  ...
  useEffect(() => {
    setLoading(true);
    api.get(`/invoices/analytics?metric=${metric}&range=${range}&groupBy=${groupBy}&chartType=${chartType}`)
  ...
  const renderChart = () => {
    if (loading) return <div className="h-64 flex items-center justify-center text-muted">Loading chart data...</div>;
    ...
    <div className="space-y-6">
      <div className="bg-surface rounded-lg border border-gray-200 p-6 flex gap-4">
        ...
        <div className="flex-1">
          ...
          <select className="input w-full" value={chartType} onChange={e => setChartType(e.target.value)}>
            <option>Bar</option>
            <option>Line</option>
            <option>Pie</option>
          </select>
      ...
      <div className="bg-surface rounded-lg border border-gray-200 p-6">
        <h3 className="text-lg font-semibold mb-6">{metric} Over {range}</h3>
        {renderChart()}
      </div>
    </div>
  );
