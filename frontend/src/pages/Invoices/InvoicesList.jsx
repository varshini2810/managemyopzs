...
export default function InvoicesList() {
  ...
  const fetchData = () => {
    ...
    api.get(`${endpoint}?page=${page}&size=20&search=${debouncedSearch}&status=${status}`)
      .then(res => {
  ...
  const invoiceColumns = [
    {
      header: 'Invoice ID',
      cell: (row) => (
        <div className="cursor-pointer hover:text-accent transition-colors" onClick={() => navigate(`/invoices/${row.id}`)}>
          <MonoId id={row.id} />
        </div>
      )
    },
    ...
    { 
      header: 'Invoice Date', 
      cell: (row) => <span className="text-muted tabular-nums">{new Date(row.invoiceDate).toLocaleDateString()}</span>
    },
    { 
      header: 'Amount Due', 
      cell: (row) => <span className="font-medium">${row.amountDue.toFixed(2)}</span>
    },
    { 
      header: 'Status', 
      cell: (row) => (
          ...
          <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 shadow-sm rounded-md py-1 min-w-[140px] hidden group-hover:block z-10 text-left">
            ...
            {!['PAID', 'VOIDED'].includes(row.status) && (
              <button 
                ...
                onClick={(e) => { e.stopPropagation(); setSelectedInvoiceId(row.id); setShowMarkPaidModal(true); }}
              >
  ...
  const creditNoteColumns = [
    ...
    { header: 'Total', cell: (row) => <span className="font-medium">${row.total.toFixed(2)}</span> },
    { header: 'Available', cell: (row) => <span className="text-green-600">${row.availableAmount.toFixed(2)}</span> },
    ...
    <div>
      <div className="module-header border-b border-gray-200">
        <div>
          ...
          <p className="page-subtitle">Manage billing documents and adjustments</p>
        </div>
        <div>
          {activeTab === 'invoices' && (
          ...
          {activeTab === 'credit-notes' && (
            <button className="btn-primary btn-sm" onClick={() => setIsCreditModalOpen(true)}>
        ...
        <div className="bg-surface rounded-lg border border-gray-200">
          <div className="flex border-b border-gray-200">
            <button
              ...
              onClick={() => { setActiveTab('invoices'); setPage(0); setSearch(''); setStatus(''); }}
            >
              Invoices
            </button>
            <button
              ...
              onClick={() => { setActiveTab('credit-notes'); setPage(0); setSearch(''); setStatus(''); }}
            >
              Credit Notes
            </button>
            <button
            ...
            <button
              ...
              onClick={() => { setActiveTab('payment-statement'); }}
            >
              Payment &amp; Statement
            </button>
          </div>
          ...
          {(activeTab === 'invoices' || activeTab === 'credit-notes') && (
            ...
            {activeTab === 'invoices' && (
              <select className="input w-48" value={status} onChange={(e) => { setStatus(e.target.value); setPage(0); }}>
            ...
            {activeTab === 'credit-notes' && (
              ...
              <DataTable 
                columns={activeTab === 'invoices' ? invoiceColumns : creditNoteColumns}
                data={data}
                loading={loading}
      ...
      <MarkAsPaidModal 
        ...
        onClose={() => { setShowMarkPaidModal(false); setSelectedInvoiceId(null); }} 
