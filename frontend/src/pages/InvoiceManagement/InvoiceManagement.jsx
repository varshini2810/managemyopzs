...
const STATUS_CFG = {
  paid: { label: "Paid", bg: "#dcfce7", color: "#166534", dot: "#22c55e" },
  ...
  void: { label: "Void", bg: "#fdf4ff", color: "#86198f", dot: "#d946ef" },
};
const mkItem = (desc, qty, price, tax = 18, disc = 0) => ({
  desc,
  qty,
  price,
  tax,
...
function StatCard({
  ...
  return (
    <div
      className="bg-white rounded-2xl p-5 flex flex-col gap-3 transition-all duration-200 cursor-default group"
      style={{
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        {trend !== undefined && (
          <div
            className={`flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${trend >= 0 ? "text-green-700 bg-green-50" : "text-red-600 bg-red-50"}`}
          >
            {" "}
            {trend >= 0 ? (
              <TrendingUp size={11} />
    ...
    onSave(
      {
        ...
        tax: (form.items || []).reduce((s, it) => {
          const l = (Number(it.qty) || 0) * (Number(it.price) || 0);
          const d = l * ((Number(it.disc) || 0) / 100);
          return s + (l - d) * ((Number(it.tax) || 0) / 100);
        }, 0),
        disc: (form.items || []).reduce((s, it) => {
          const l = (Number(it.qty) || 0) * (Number(it.price) || 0);
          return s + l * ((Number(it.disc) || 0) / 100);
        }, 0),
                ...
                <div className="w-72 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between font-bold text-gray-900 text-base pt-2 border-t border-gray-200">
                    ...
                    <span className="tabular-nums text-indigo-700">
                      {" "}
                      {fmt(totals.grandTotal, form.currency)}{" "}
                    </span>{" "}
                  </div>{" "}
                </div>{" "}
              </div>{" "}
        ...
        <div className="flex-1 overflow-y-auto px-7 py-6 space-y-6">
          ...
          <div className="flex justify-end">
            ...
            <div className="w-64 space-y-2 text-sm">
              {" "}
              {[
                ["Subtotal", fmt(inv.subtotal || 0, inv.currency)],
                ["Tax", fmt(inv.tax || 0, inv.currency)],
                ["Discount", "-" + fmt(inv.disc || 0, inv.currency)],
                ["Shipping", fmt(inv.shipping || 0, inv.currency)],
              ].map(([k, v]) => (
                <div key={k} className="flex justify-between text-gray-500">
                  {" "}
                  <span>{k}</span>{" "}
              ...
              <div className="flex justify-between text-green-600 font-medium">
                ...
                <span className="tabular-nums">
                  {" "}
                  {fmt(inv.paidAmt || 0, inv.currency)}{" "}
                </span>{" "}
              </div>{" "}
              <div className="flex justify-between font-bold text-red-600 text-base border-t border-gray-200 pt-2">
                ...
                <span className="tabular-nums">
                  {" "}
                  {fmt(Math.max(0, balance), inv.currency)}{" "}
                </span>{" "}
              </div>{" "}
            </div>{" "}
          </div>{" "}
...
function PaymentModal({ open, invoice: inv, onClose, onConfirm }) {
  ...
  useEffect(() => {
    if (open && inv) setAmount(String(balance));
  }, [open, inv]);
  if (!open || !inv) return null;
  return (
    <div
      ...
      >
        ...
        <div className="px-6 py-7 text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">
            ...
            <strong className="text-gray-800">{inv.id}</strong> for{" "}
            <strong className="text-gray-800">{inv.client}</strong>. This action
            cannot be undone.{" "}
          </p>{" "}
          <div className="text-left">
            ...
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">
              {" "}
              Type <span className="font-mono text-red-500">{inv.id}</span> to
              confirm{" "}
            </label>{" "}
            <input
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
          ...
          <button
            ...
            style={{
              background: "#DC2626",
              boxShadow:
                confirm === inv.id ? "0 4px 14px rgba(220,38,38,0.3)" : "none",
            }}
          >
            {" "}
            Delete Invoice{" "}
...
function exportCSV(data) {
  ...
  const rows = data.map((r) => [
    ...
    r.total,
    r.paidAmt || 0,
    Math.max(0, (r.total || 0) - (r.paidAmt || 0)),
    r.status,
    r.payMethod,
    r.createdBy,
  ]);
...
export default function InvoiceManagement() {
  ...
  useEffect(() => {
    setPage(0);
    setSelected(new Set());
  }, [search, statusFilter, dateFrom, dateTo, payMethodFilter, currencyFilter]);
  const filtered = useMemo(() => {
    const q = search.toLowerCase().trim();
    return invoices.filter(
      (inv) =>
        (!q ||
          [inv.id, inv.client, inv.company, inv.email, inv.phone].some((v) =>
    ...
    () =>
      [...filtered].sort((a, b) => {
        let av = a[sort.field],
          bv = b[sort.field];
        if (
          ["total", "subtotal", "paidAmt", "tax", "disc"].includes(sort.field)
        ) {
          av = Number(av || 0);
          bv = Number(bv || 0);
        }
  ...
  const stats = useMemo(
    () => ({
      ...
      outstanding: invoices
        ...
        .reduce(
          (s, i) => s + Math.max(0, (i.total || 0) - (i.paidAmt || 0)),
          0,
        ),
      avgValue: invoices.length
  ...
  const handleMarkPaid = (inv) => {
    setInvoices((prev) =>
      prev.map((i) =>
        i.id === inv.id ? { ...i, status: "paid", paidAmt: i.total } : i,
      ),
    );
  ...
  const handleVoid = (inv) => {
    setInvoices((prev) =>
      prev.map((i) => (i.id === inv.id ? { ...i, status: "void" } : i)),
    );
  ...
  const handleBulkExport = () => {
    exportCSV(sorted.filter((i) => selected.has(i.id)));
    toast.success("Exported " + selected.size + " invoice(s)");
  };
  const handleBulkEmail = () => {
    toast.success("Emails queued for " + selected.size + " invoice(s)");
    setSelected(new Set());
  };
  const handleBulkMarkPaid = () => {
    setInvoices((prev) =>
      prev.map((i) =>
        selected.has(i.id) ? { ...i, status: "paid", paidAmt: i.total } : i,
      ),
    );
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(160deg, #EEF2FF 0%, #F0F9FF 50%, #F5F3FF 100%)",
      }}
    >
      {" "}
      {}{" "}
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">
              Manage, track and send all your customer invoices
            </p>{" "}
          </div>{" "}
          <div className="flex items-center gap-3">
            {" "}
      ...
      <div className="px-8 pb-8 space-y-6">
        ...
        <div className="grid grid-cols-3 lg:grid-cols-5 xl:grid-cols-9 gap-3">
          ...
          {[
            ...
            {
              title: "Draft",
              rawValue: stats.draft,
              displayValue: String(stats.draft),
              icon: FileText,
              color: "#6B7280",
              bg: "#F3F4F6",
            },
            {
              title: "Cancelled",
              rawValue: stats.cancelled,
              displayValue: String(stats.cancelled),
              icon: XCircle,
              color: "#9333EA",
              bg: "#FDF4FF",
            },
            {
              ...
              displayValue: "$" + (stats.revenue / 1000).toFixed(1) + "k",
              icon: DollarSign,
              color: "#16A34A",
              bg: "#F0FDF4",
              trend: 15,
            ...
            {
              title: "Outstanding",
              rawValue: Math.round(stats.outstanding),
              displayValue: "$" + (stats.outstanding / 1000).toFixed(1) + "k",
              icon: TrendingDown,
              color: "#DC2626",
              bg: "#FEF2F2",
              trend: -8,
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-max">
              <thead>
                ...
                >
                  <th
                    className="px-4 py-3.5 w-10 sticky left-0"
                    style={{ background: "#F8FAFC" }}
                  >
                    <div
                  ...
                  {colVisible("balance") && (
                    <th
                      className="px-4 py-3.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap"
                      style={{ background: "#F8FAFC" }}
                    >
                      Balance
                    </th>
                  ...
                  pageData.map((row, ri) => {
                    ...
                    return (
                      ...
                      >
                          ...
                          <td className="px-4 py-3.5">
                            <div className="flex items-center gap-2.5">
                              <div
                                ...
                                style={{
                                  background: `hsl(${(row.client.charCodeAt(0) * 37) % 360}, 60%, 55%)`,
                                }}
                              >
                                {" "}
                                {row.client.charAt(0)}{" "}
                        ...
                        {colVisible("issueDate") && (
                          <td className="px-4 py-3.5 text-sm text-gray-600 whitespace-nowrap tabular-nums">
                        ...
                        {colVisible("subtotal") && (
                          <td className="px-4 py-3.5 text-sm text-right tabular-nums text-gray-600">
                            {fmt(row.subtotal, row.currency)}
                          </td>
                        )}{" "}
                        {colVisible("tax") && (
                          <td className="px-4 py-3.5 text-sm text-right tabular-nums text-gray-500">
                            {fmt(row.tax || 0, row.currency)}
                          </td>
                        )}{" "}
                        {colVisible("disc") && (
                          <td className="px-4 py-3.5 text-sm text-right tabular-nums text-orange-500">
                            {row.disc > 0
                              ? "-" + fmt(row.disc, row.currency)
                              : "—"}
                          </td>
                        )}{" "}
                        {colVisible("total") && (
                          <td className="px-4 py-3.5 text-sm text-right tabular-nums font-bold text-gray-900">
                            {fmt(row.total, row.currency)}
                          </td>
                        )}{" "}
                        {colVisible("paidAmt") && (
                          <td className="px-4 py-3.5 text-sm text-right tabular-nums text-green-600 font-medium">
                            {row.paidAmt > 0
                              ? fmt(row.paidAmt, row.currency)
                              : "—"}
                          </td>
                        )}{" "}
                        {colVisible("balance") && (
                          <td
                            className="px-4 py-3.5 text-sm text-right tabular-nums font-semibold"
                            style={{
