...
export default function MetricExplorer() {
  ...
  const handleApply = () => {
    ...
    api.get('/revenue-story/dashboard')
      .then(res => {
        // We reuse the dashboard chartData for demonstration
          ...
          <tbody>
            {data.map((row, idx) => (
              <tr key={idx} className="border-b border-gray-100">
                ...
                <td className="py-2 text-right">{row[dataKey]}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6 animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Flexible analysis for all your revenue metrics.</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="btn-secondary"><Save size={14} /> Save as Custom Metric</button>
      ...
      <div className="card min-h-[400px] flex flex-col">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="seg-control">
            <button className={`seg-btn ${chartType === 'Line' ? 'active' : ''}`} onClick={() => setChartType('Line')}>Line</button>
            <button className={`seg-btn ${chartType === 'Bar' ? 'active' : ''}`} onClick={() => setChartType('Bar')}>Bar</button>
            <button className={`seg-btn ${chartType === 'Table' ? 'active' : ''}`} onClick={() => setChartType('Table')}>Table</button>
          </div>
        </div>
        <div className={`flex-1 flex items-center justify-center ${!data ? 'text-muted border border-dashed border-border rounded' : ''} min-h-[300px]`}>
          {renderChart()}
        </div>
      </div>
    </div>
