...
import { Search, ChevronLeft, ChevronRight, Trash2, Archive } from 'lucide-react';
      ...
      {columns.map((col, ci) => (
        <td key={ci} className="px-4 py-3">
          <div
            ...
            style={{
              height: 14,
              width: col.skeletonWidth || (ci === 0 ? '60%' : ci === columns.length - 1 ? '50px' : '80%'),
              background: '#E7E5E2',
            ...
            {loading ? (
              ...
              data.map((row, ri) => (
                <tr
                  ...
                  onMouseEnter={() => !showCheck && selected.size === 0 && undefined}
                >
                  {(showCheck || selected.size > 0) && (
                    <td
      ...
      {!loading && totalElements > 0 && (
        ...
        >
          <span className="tabular-nums">
            Showing <strong className="text-ink">{startRow}–{endRow}</strong>{' '}
            of <strong className="text-ink">{totalElements.toLocaleString()}</strong>
          </span>
          <div className="flex items-center gap-1">
            ...
            <span className="px-2 tabular-nums" style={{ minWidth: 80, textAlign: 'center' }}>
      ...
      {selected.size > 0 && (
        <div
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50
                     flex items-center gap-3 px-4 py-3 bg-ink text-white rounded-lg shadow-tooltip"
