...
export async function generateInvoicePdf(invoice, customer = null) {
  ...
  const custName = customer?.companyName
  ...
  if (customer?.email)       { doc.text(customer.email, toX, ty); ty += 11; }
  if (customer?.phone)       { doc.text(customer.phone, toX, ty); ty += 11; }
  if (customer?.vatNumber)   { doc.text(`GST: ${customer.vatNumber}`, toX, ty); ty += 11; }
