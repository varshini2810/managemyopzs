...
@Repository
interface ChargeRepository extends JpaRepository<Charge, String> {
    @Query("SELECT c FROM Charge c WHERE c.deletedAt IS NULL AND (LOWER(c.name) LIKE LOWER(CONCAT('%',:search,'%')))")
    Page<Charge> findAllActive(String search, Pageable pageable);
    Optional<Charge> findByIdAndDeletedAtIsNull(String id);
}
