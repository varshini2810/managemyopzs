...
interface PlanRepository extends JpaRepository<Plan, String> {
    @Query("SELECT p FROM Plan p WHERE p.deletedAt IS NULL AND (LOWER(p.internalName) LIKE LOWER(CONCAT('%',:search,'%')) OR LOWER(p.id) LIKE LOWER(CONCAT('%',:search,'%')))")
    Page<Plan> findAllActive(String search, Pageable pageable);
    Optional<Plan> findByIdAndDeletedAtIsNull(String id);
}

@Repository
interface PlanPricePointRepository extends JpaRepository<PlanPricePoint, String> {
    List<PlanPricePoint> findByPlanIdAndDeletedAtIsNull(String planId);
}
