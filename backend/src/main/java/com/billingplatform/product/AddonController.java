...
@Repository
interface AddonRepository extends JpaRepository<Addon, String> {
    @Query("SELECT a FROM Addon a WHERE a.deletedAt IS NULL AND (LOWER(a.name) LIKE LOWER(CONCAT('%',:search,'%')))")
    Page<Addon> findAllActive(String search, Pageable pageable);
    Optional<Addon> findByIdAndDeletedAtIsNull(String id);
}
