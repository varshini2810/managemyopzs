package com.billingplatform.service;

import com.billingplatform.model.*;
import com.billingplatform.repository.*;
import com.billingplatform.dto.*;
import com.billingplatform.security.*;
import com.billingplatform.service.*;
import com.billingplatform.exception.*;
import com.billingplatform.util.*;


import com.billingplatform.model.UserRoleAssignment;
import com.billingplatform.repository.UserRoleAssignmentRepository;
import org.springframework.stereotype.Service;
import org.springframework.beans.factory.annotation.Autowired;
import java.util.List;

@Service
public class UserRoleAssignmentService {
    @Autowired
    private UserRoleAssignmentRepository repository;

    public List<UserRoleAssignment> findAll() { return repository.findAll(); }
    public UserRoleAssignment save(UserRoleAssignment entity) { return repository.save(entity); }
}
